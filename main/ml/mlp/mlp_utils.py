...
def train(X_train, y_train, epochs, lr, hidden_layers, probabilistic, path=None, device="cpu", verbose=False):
    ...
    if path is not None:
        ...
        joblib.dump(model.x_scaler, path/"minmax_scaler.pkl")
...
def run_pipeline(data_path, result_path, params, probabilistic, log_y=False, verbose=False):
    ...
    if log_y:
        model = train(X_train, np.log(y_train), epochs, lr, hidden_layers, probabilistic, result_path, "cpu", verbose)
    else:
        model = train(X_train, y_train, epochs, lr, hidden_layers, probabilistic, result_path, "cpu", verbose)
    ...
    plot_model_timelines(model, X_timeline, X_train, y_train, plot_path, quantiles=True)
